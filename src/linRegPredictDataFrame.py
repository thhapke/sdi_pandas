...
def process(msg,coef_msg):
    ...
    if api.config.regresssion_cols_value and api.config.regresssion_cols_value.upper() != 'NONE' :
        colmap = [x.strip() for x in api.config.regresssion_cols_value.split(',')]
        valmap = {cm.split(':')[0].strip().strip("'").strip('"'):cm.split(':')[1].strip().strip("'").strip('"') for cm in colmap}
...
try:
    ...
    class api:
        ...
        def set_config(test_scenario) :
            ...
            api.config.regression_cols_value = 'None'
