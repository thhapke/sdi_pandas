...
def process(msg):
    ...
    if not api.config.df_name or api.config.df_name.upper() == 'NONE':
        att_dict['name'] = "DataFrame"
    else :
    ...
    if api.config.col_date_name and api.config.col_date_name.upper() == 'NONE':
        dgr = re.match('(\d+)-(\d+)-(\d+).*', os.path.basename(csv_file))
        y = int(dgr.group(1))
        m = int(dgr.group(2))
        d = int(dgr.group(3))
        df[api.config.col_date_name] = datetime.date(year=y, month=m, day=d)
    if api.config.drop_col1 :
...
try:
    ...
    class api:
        def set_test(test_scenario):
            ...
            if test_scenario == test.VERIVOX :
                f = open("/Users/d051079/OneDrive - SAP SE/GitHub/OptRanking/data/verivox_archive/Strom_Privattarife_Empfehlungen_5000kWh_2019_05_27.csv.gz", mode='rb')
                bcsv = f.read()
            else:  # testdata.BINARY_STRING
                bcsv = b"""col1;col2;col3
                         1;4.4;99
                         2;4.5;200
                         3;4.7;65
                         4;3.2;140
            ...
            return api.Message(attributes=attributes, body=bcsv)

        def set_config(test_scenario):
            if test_scenario == test.VERIVOX:
                api.config.index_cols = ""
                ...
                api.config.df_name = 'verivox'
                api.config.decimal = ','
                api.config.drop_col1 = True
        ...
        def send(port, msg):
            if isinstance(msg, str):
                ...
                msg.body.to_csv("testunzip.csv",sep=';')
            pass
