...
def process(msg):
    ...
    att_dict['storage.endOfSequence'] = msg.attributes['storage.endOfSequence']

    example_rows = EXAMPLE_ROWS if att_dict['number_rows'] > EXAMPLE_ROWS else att_dict['number_rows']
    for i in range(0,example_rows) :
        att_dict['row_'+str(i)] = str([ str(i)[:10].ljust(10) for i in result_df.iloc[i, :].tolist()])

    # if a single file is read or the end of a list of file the result df is returned
    if msg.attributes['storage.endOfSequence'] :
        return True, api.Message(attributes=att_dict, body=result_df)
    else :   
        return False, api.Message(attributes=att_dict, body=result_df)
)
...
def interface(msg):
    sendDF, result = process(msg)
    # only send msg if all files are read
    if sendDF :
