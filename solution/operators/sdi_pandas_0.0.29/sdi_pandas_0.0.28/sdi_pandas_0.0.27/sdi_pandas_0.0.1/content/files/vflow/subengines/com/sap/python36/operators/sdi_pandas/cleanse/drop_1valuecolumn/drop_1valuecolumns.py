...
try:
    ...
    class api:
        class config:
            ...
            operator_description = 'Drop 1 Value Columns'
            operator_description_long='Drops columns of DataFrame with only 1 unique value.'
...
def process(msg):
    ...
    col1val_data = {'column': [], 'type': [], 'unique_vals': []}
    for col in columns:
        vals = df[col].unique()
        if len(vals) == 1:
