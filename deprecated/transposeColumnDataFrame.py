...
def process(msg):
    ...
    if api.config.selected_values or not api.config.selected_values == "None" :
        ...
        att_dict['config']['selected_values'] = str(selected_values)
    else :
        selected_values = df[api.config.transpose_column].unique()
    if len(selected_values) > MAX_COLUMNS :
        selected_values = selected_values[:MAX_COLUMNS]
        att_dict['warning'] = att_dict['warning'] + ";Truncated the columns to MAX_COLUMNS: " + str(MAX_COLUMNS + ' ')
    att_dict['config']['selected_values'] = str(selected_values)
    ...
    att_dict['config']['groupby_column'] = groupby_column
