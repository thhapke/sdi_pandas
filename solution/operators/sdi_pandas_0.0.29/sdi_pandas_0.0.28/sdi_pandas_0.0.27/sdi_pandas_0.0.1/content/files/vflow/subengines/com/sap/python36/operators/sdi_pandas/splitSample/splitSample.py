...
def process(msg):
    ...
    return train_msg, test_msg, log_stream.getvalue()

inports = [{"name":"input","type":"message","description":"Input data"}]
outports = [{"name":"train","type":"message","description":"train sample"},\
            {"name":"test","type":"message","description":"test sample"},\
            {"name":"log","type":"string","description":"Logging"}]

def call_on_input(msg) :
    train_msg, test_msg, log = process(msg)
    api.send(outports[0]['name'], train_msg)
    api.send(outports[1]['name'], test_msg)
    api.send(outports[2]['name'], log)
...
def main() :
    ...
    train_msg, test_msg, log = api.call(config,input_msg)
    print('Input')
