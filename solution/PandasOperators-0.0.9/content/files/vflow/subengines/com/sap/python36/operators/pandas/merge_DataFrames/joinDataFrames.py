...
def process(left_msg,right_msg):
    ...
    how = api.config.how.strip().replace("'",'').replace('"','')
    ...
    if api.config.on_index :
        ...
        left_on_list = [x.strip().replace("'",'').replace('"','') for x in api.config.left_on.split(',')]
        right_on_list = [x.strip().replace("'", '').replace('"', '') for x in api.config.right_on.split(',')]
    ...
    if api.config.new_indices and not api.config.new_indices == 'None':
        ...
        index_list = [x.strip().replace("'",'').replace('"','') for x in api.config.new_indices.split(',')]
...
class test :
    ...
    ORDER_PRODUCT_MD = 4
    CHECK24_VERIVOX = 5
test_scenario = test.CHECK24_VERIVOX
...
try:
    ...
    class api:
        ...
        def set_test(test_scenario):
            if test_scenario in [test.READ_BIG, test.ORDER_HEADER_DETAIL, test.ORDER_PRODUCT_MD] :
            ...
            elif test_scenario == test.CHECK24_VERIVOX :
                l_df = pd.read_csv("/Users/d051079/OneDrive - SAP SE/GitHub/OptRanking/data/verivox/sample_verivox_p1.csv",sep=';')
                r_df = pd.read_csv("/Users/d051079/OneDrive - SAP SE/GitHub/OptRanking/data/check24/sample_check24_p1.csv",sep=';')
            else :
                l_df = pd.DataFrame(
        ...
        def set_config (test_scenario) :
            if test_scenario == test.ORDER_HEADER_DETAIL :
                ...
                api.config.how = "inner"
                api.config.on_index = False
                api.config.left_on ="'Datum','Postleitzahl','Ort','Verbrauchsstufe'"
                api.config.right_on = "'Datum','Postleitzahl','Ort','Verbrauchsstufe'"
                api.config.new_indices ="None"
                api.config.drop_columns = 'None'
            else :
                api.config.how = "inner"
                api.config.on_index = False
